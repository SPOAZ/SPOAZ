...
def genome_mapping(genome):
    genome.sort_values(by=["track_id"], inplace=True)
    track_genome = genome.groupby("track_id")["relevance"].agg(list).reset_index()

    track_genome = {a: b for a, b in zip(track_genome['track_id'], track_genome['relevance'])}
